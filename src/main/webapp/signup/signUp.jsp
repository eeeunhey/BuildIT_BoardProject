...
<body>
	<div class="register-container">
		<h2>회원가입</h2>
		<form action="signUpProcess.do" method="post" class="register-form"
			onsubmit="return validateForm()">

			<!-- 아이디 -->
			<label for="id">아이디</label> <input type="text" id="id" name="id"
				placeholder="아이디" required onblur="checkId()"> <span
				id="idCheckResult"
				style="font-size: 12px; display: block; margin-bottom: 10px;"></span>
			...
			<label for="password">비밀번호</label> <input type="password"
				id="password" name="password" placeholder="비밀번호 (8자 이상 32자 이하)"
			...
			<div class="radio-group">
				<p>회원 유형</p>
				<label> <input type="radio" name="type" value="CLIENT"
					required> CLIENT (프로젝트 리더)
				</label> <label> <input type="radio" name="type" value="PARTNER"
					required> PARTNER
				</label>
			</div>
			...
			<button type="button" class="btn-social google"
				onclick="alert('구글 가입은 현재 준비 중입니다.')">구글로 회원가입</button>
			<button type="button" class="btn-social facebook"
				onclick="alert('카카오톡 가입은 현재 준비 중입니다.')">카카오톡으로 회원가입</button>

			<p class="login-text">
				통합회원이신가요? <a href="/login/login.do">로그인</a>
			</p>
			...
			%>
			<script>alert('<%=flash.replace("'", "\\'")%>');</script>
			<%
			}
			%>
		</form>
